...
NDiplomacy = {
	...
	BASE_PEACE_FORCE_GOVERNMENT_COST = 10000, 		-- Base cost for forcing a country to change government.
	...
	PEACE_COST_FACTOR_STACK_WAR_REPARATION = 0.5,
	...
	PEACE_TIMED_EFFECT_LENGTH_DEMILITARIZED_ZONE = 1826, -- 5 years
	PEACE_TIMED_EFFECT_LENGTH_WAR_REPARATION = 1826,
	PEACE_TIMED_EFFECT_LENGTH_RESOURCE_RIGHTS = 1826,
	PEACE_TIMED_EFFECT_RATIO_CIVILIAN_FACTORY_WAR_REPARATION = 1, 	-- ratio of civilian factories taken via stackable war reparation
	...
	BASE_SEND_ATTACHE_COST = 200,					-- Political power cost to send attache
	BASE_SEND_ATTACHE_CP_COST = 60.0,				-- Command Power sent attache usage cost
	BASE_GENERATE_WARGOAL_DAILY_PP = 1,	-- Daily pp cost for generation of wargoals
	...
	BASE_BOOST_PARTY_POPULARITY_DAILY_PP = 1, 	-- Daily pp cost for boost party popularity
	BASE_BOOST_PARTY_POPULARITY_DAILY_DRIFT = 0.4, 	-- Daily amount of popularity that will be added by the activity.
	BASE_STAGE_COUP_DAILY_PP = 1,					-- Daily pp cost for staging a coup
	BASE_STAGE_COUP_TOTAL_COST = 20000, 				-- Equipment consume factor for stage coup.
	...
	TROOP_FEAR = 10, 								-- Impact on troops on borders when deciding how willing a nation is to trade
	FLEET_FEAR = 10,									-- Impact on troops on borders when deciding how willing a nation is to trade
	...
	TENSION_WAR_REPARATION = 0.5,					-- Amount of tension generated by stacking war reparation on 1 state, multiplied with the state base threat
	...
	BASE_CONDITIONAL_PEACE_MONTHS = 1,				-- War length must be before a surrender is possible.
	...
	PEACE_SCORE_SCALE_FACTOR = 1.5,                -- Losers' total value times this factor becomes the default total peace conference score that is distributed to the winners.
	...
	PEACE_SCORE_DISTRIBUTION = { 0.5, 0.4, 0.3, 0.2, 0.1 }, -- How much of the total peace conference score you get during the first n turns.
	...
	MAX_REMEMBERED_LEASED_IC = 10000,				-- Maximum of leased equipment value that is remembered for opinion bonus
	MAX_OPINION_FOR_LEASED_IC = 100,					-- Positive opinion when remembering the MAX_REMEMBERED_LEASED_IC equipment
	MONTHLY_LEASED_IC_DECAY = 200,					-- How much of leased equipment is being "forgot" each month
	OPINION_PER_VOLUNTEER = 10,						-- Opinion bonus per one sent volunteer division
	MAX_OPINION_FROM_VOLUNTEERS = 200,				-- Opinion bonus per one sent volunteer division
	...
	RESOURCE_SENT_AUTONOMY_DAILY_FACTOR = 0.0,	-- If puppet provides resources to its master they increasy their autonomy by the resources factored by this
	WAR_SCORE_AUTONOMY_BASE = 0.0,					-- Value added if any war score is contributed by puppet
	WAR_SCORE_AUTONOMY_FACTOR = 1.0,				-- If puppet generates war score it get a boost to independence
	...
	AUTONOMY_FREEDOM_FROM_CAPITULATE = 10,         -- if overlord capitulate you get this
	ATTACHE_TO_SUBJECT_EFFECT = 0.0,				-- If overlord sent attaches to the subject it losses autonomy
	ATTACHE_TO_OVERLORD_EFFECT = 0.0,				-- If subject sent attaches to the overlord it gains autonomy

	AUTONOMY_LEVEL_CHANGE_PP_COST_BASE = 50.0,		-- Base cost of changing level of autonomy
	AUTONOMY_LEVEL_CHANGE_PP_ANNEX = 3000,			-- Annexation cost
	...
	ASSUME_FACTION_LEADERSHIP_PP_COST = 3000,				-- Political power cost to assume faction leadership
	...
	FACTION_LEADERSHIP_CHANGE_ALERT_THRESHOLD = 0.95,		-- Threshold for showing an alert when a faction member is close to being able to assume leadership of the faction that a player currently leads.
...
NCountry = {
	...
	AIR_SCORE_MULTIPLIER = 0.01,						-- Based on number of planes (which is typically a lot).
	...
	SUPPLY_CONVOY_FACTOR = 1,					-- How many convoys each supply needs
	...
	ADDITIONAL_MAJOR_COUNTRIES_IC_RATIO = 0.35,		-- Countries will also be considered major when having more factories that the average of top MIN_MAJOR_COUNTRIES countries' factories times ADDITIONAL_MAJOR_COUNTRIES_IC_RATIO
	BASE_TENSION_MAJOR_COUNTRY_INDEX = 0,			-- Which major country should be considered the base country when scaling generated world tension. 0 is the country with the most factories, 1 is the second most-factories country etc. This number has to be lower than MIN_MAJOR_COUNTRIES
	...
	INVASION_REPORT_EXPERATION_DAYS = 14,			-- Invasion experation days
	...
	CIVIL_WAR_INVOLVEMENT_MIN_TENSION = 1,		-- base value of world tension to involve other sides to the civil war
	...
	MIN_SURRENDER_LIMIT = 0.05,						-- Minimum non-forced surrender limit. valid 0-1
	...
	DAYS_OF_WAR_BEFORE_SURRENDER = 3,				-- Number of days a war has to have existed before anyone can surrender in it

	FUEL_LEASE_CONVOY_RATIO = 0.005,				-- num convoys needed per fuel land lease
	...
	SCORCHED_EARTH_STATE_COST = 25,					-- pp cost to scorch a state
	...
	COUNTRY_MANPOWER_CAPITULATED_NON_CORE_GAIN_FACTOR = 0.0,	-- Factor on amount of normal manpower gained for the exile nation. From owned states that are controlled by an enemy. State manpower reduced by factor 1000 in code.
...
NResistance = {
	...
	RESISTANCE_TARGET_BASE = 70.0,							-- base resistance target percentage

	RESISTANCE_TARGET_MODIFIER_HAS_CLAIM = -40.0,			-- resistance target modifier in % for states we have claim
	...
	RESISTANCE_TARGET_MODIFIER_OCCUPIED_CAPITULATED = 15.0, -- resistance target modifier when the enemy is capitulated

	RESISTANCE_TARGET_MODIFIER_OCCUPIED_IS_EXILE_MIN = 3.0,   -- min & max resistance target modifier resistance target modifier for exile countries. interpolated using legitimacy
	RESISTANCE_TARGET_MODIFIER_OCCUPIED_IS_EXILE_MAX = 30.0,

	RESISTANCE_TARGET_MODIFIER_POP_LOW = -30.0,			-- how much we reduce the resistance target
	RESISTANCE_TARGET_MODIFIER_POP_VERY_LOW = -100.0,			-- resistance target modifier in % for states we have claim
	...
	RESISTANCE_DECAY_BASE = 0.2, -- base resistance decay
	...
	RESISTANCE_TARGET_TO_REENABLE_RESISTANCE = 20, -- resistance will be disabled once it reaches zero and will not be reenabled until resistance target reaches above this value
...
NProduction = {
	...
	MAX_CIV_FACTORIES_PER_LINE = 150,	-- Max number of factories that can be assigned a single production line.
...
NTechnology = {
	...
	BASE_RESEARCH_POINTS_SAVED = 90.0,		-- Base amount of research points a country can save per slot.
	...
	MAX_TECH_SHARING_BONUS = 0.9, 			-- Max technology sharing bonus that can be applied instantly
	...
	MIN_RESEARCH_SPEED = 0.05,						-- research speed can't go below this value
...
NOperatives = {
	...
	BECOME_SPYMASTER_PP_COST = 100,					-- Number of political power used to become Spy Master
	...
	OPERATIVE_BASE_ROOT_OUT_RESISTANCE_EFFICIENCY = 5.0,			-- The base efficiency of an operative at the RootOutResistance mission (this is a percentage, 1.0 == 100%)
